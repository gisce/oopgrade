...
with description('clean_requirements_lines') as self:
    ...
    with context('with pymongo variants'):
        with it('removes generic if all marker-specific lines have the same version'):
            input_lines = [
                "pymongo<=3.13.0",
                "pymongo==3.13.0 ; python_version<=\"2.7.18\"",
                "pymongo==3.13.0 ; python_version>\"2.7.18\"",
            ...
            expect(result).not_to(contain('pymongo<=3.13.0'))

    with context('with rq variants'):
        with it('removes unversioned generic if marker-specific lines cover the space'):
            input_lines = [
                "rq",
                "rq==1.3.0 ; python_version<=\"2.7.18\"",
                "rq==1.16.2 ; python_version>\"2.7.18\"",
            ...
            expect(result).not_to(contain('rq'))